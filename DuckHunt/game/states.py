...
FONT = os.path.join(fullPath, 'arcadeclassic.ttf')
...
class BaseState(object):
    def __init__(self):
        ...
        self.hitDucks = [False for i in range(config_master.testnum)]
        self.hitDuckIndex = 0
            ...
            if event.button == 2:
                ...
                for duck in self.ducks:
                    if hasFired and duck.isShot(event.pos):
                        #score count
                        self.registry.set('score', self.registry.get('score') + config_master.score)
...
class RoundEndState(BaseState):
    def __init__(self, hitDucks):
        ...
        if missedCount >= config_master.miss:
